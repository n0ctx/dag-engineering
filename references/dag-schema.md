...
- `.dag/dag.json` is sealed. Its checksum lives beside it in `.dag/.dag.json.seal`, written on every runtime write and verified on every read, so a control file edited with a file tool fails closed instead of carrying state no gate approved. Commit the seal with the DAG. A deliberate repair, such as resolving a merge conflict, is re-sealed with `update-task --reseal --reason "<why>"`, which records the repair in `reseal` for the audit trail. Archived copies are records, not control files, and carry no seal.
- An unapproved plan is corrected with `update-task --amend <draft>`, which keeps the same `dag_id`, clears `decomposition_review`, and refuses once the plan is approved or any node has attempts. Abandoning is for dropping an effort, not for editing one.
