...
- A plan that is not `approved` is replaced with `update-task --amend <draft>`, which is how review findings get fixed. It preserves what already ran: a node with attempts keeps its `status`, `attempts`, and `handoff` and cannot be dropped, and a `done` node keeps its contract too, because its diff was accepted against it. Everything unstarted may be added, dropped, and re-sliced. Amending clears `decomposition_review`, leaves the plan unapproved, and is refused while a node is running. An approved plan reaches this path through `update-task --planning-status replan_required --reference "<why>"`. Abandoning is for dropping an effort, not for editing one.
