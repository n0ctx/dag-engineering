...
- The controller owns planning, scheduling, state transitions, review coordination, independent verification, integration, and final convergence. A node's execution is bounded: one worker, one fix-first reviewer round, and — whenever anything is still open after that — one controller close-out in which the controller itself judges, repairs, commits, and records through the same gates. A node never bounces between worker and reviewer for repeated rework. Planning runs the same single loop at any stage: draft the plan, install or change it (`--init` or `--revise`), one fix-first decomposition review, one controller close-out of what the review left open, and the user's approval gate — a changed plan re-enters the loop instead of earning a narrower repair pass.
